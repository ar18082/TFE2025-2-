...
class ImmeublesController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $client = Client::with([
            'gerantImms.contacts',
            'codePostelbs',
            'appartements.relApps'=> function($query){
                $query->OrderBy('DatRel', 'desc')
                ->get();
            },
            'clichaufs',
            'contacts',
            'cliEaus',
            'appartements.relChaufApps' => function($query){
                $query->latest('DatRel')
                ->get();
            },
            'appartements.relEauApps' => function($query){
                $query->latest('DatRel')
               ->get();
            },
        ])
...
//        dd($event);
        ...
        $eventsPlanify = Event::where('client_id', $client->id)
            ->where('valide', false)
            ->orwhere('start', '1900-01-01 00:00:00')
            ->orderBy('start', 'asc')->get();

        $documents = Document::where('client_id', $client->id)
            ->with('event.typeEvent')
